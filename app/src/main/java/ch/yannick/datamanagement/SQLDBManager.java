package ch.yannick.datamanagement;
...
public class SQLDBManager extends SQLiteOpenHelper {
    ...
    public static final String KEY_TYPE= "type", KEY_PENETRATION="penetration", KEY_DIRECT="isDirect",
    		KEY_SCHADEN="schaden";
    ...
    public synchronized Long pushWeapon(Weapon w){
    	...
    	c.put(KEY_PENETRATION,""+w.penetration());
        c.put(KEY_DIRECT,w.isDirect());
    	c.put(KEY_SCHADEN,""+w.getSchaden());
    	...
    	ArrayList<String> entrys = new ArrayList<String>(w.schadenW().size());
    	for(Dice dice :w.schadenW()){
    		entrys.add(dice.name());
    	}
    	this.multiPush(TABLE_SCHADEN,id,KEY_NAME, entrys,false);
        ...
        for(Action action:w.getType().getActions()){
            ...
            contentValues.put(KEY_ATTRIBUTE1,w.getAttributes(action).get(0).name());
            contentValues.put(KEY_ATTRIBUTE2, w.getAttributes(action).get(1).name());
            contentValues.put(KEY_VALUE,w.getEnhancer(action));
            contentValues.put(KEY_FATIGUE,w.getFatigue(action));
	...
	public synchronized ArrayList<Weapon> getAllWeapon() {
    	...
    	db = getReadableDatabase();
    	Cursor cursor = db.query(TABLE_WEAPONS,new String[]{KEY_ID,KEY_NAME, KEY_TYPE,KEY_PENETRATION,KEY_SCHADEN},null,null,null,null,null);
    	...
    	if(cursor.moveToFirst()){
    		do{
                ...
                list.add(new Weapon(cursor.getLong(0), cursor.getString(1), WaffenTyp.valueOf(cursor.getString(2)),
                        get(cursor,KEY_PENETRATION) == -1,get(cursor,KEY_PENETRATION),get(cursor,KEY_SCHADEN)));
    		}while(cursor.moveToNext());
    	}
    	db.close();
    	cursor.close();
	...
	public synchronized Weapon getWeapon(Long id) throws Exception {
		Weapon w;
    	db = getReadableDatabase();
    	Cursor c = db.query(TABLE_WEAPONS, new String[]{KEY_ID,KEY_NAME,KEY_TYPE,KEY_PENETRATION,KEY_DIRECT,
    			KEY_SCHADEN,KEY_WEIGHT},
    			KEY_ID + "=?",new String[]{String.valueOf(id) }, null,null,null,null);
    	if(c.moveToFirst()){
			Log.d(LOG,"The result of the query is (name="+c.getString(1)+"/id:"+ c.getInt(0)+"/type:"+c.getString(2)+"rest"+
					c.getInt(3)+c.getInt(4)+")");


            w =  new Weapon(getLong(c,KEY_ID),getString(c,KEY_NAME),WaffenTyp.valueOf(getString(c,KEY_TYPE)),
                    getBool(c, KEY_DIRECT),get(c,KEY_PENETRATION),get(c,KEY_SCHADEN));
            w.setWeight(get(c,KEY_WEIGHT));
			c.close();

            // Schaden
	    	c = db.query(TABLE_SCHADEN, new String[]{KEY_ID,KEY_NAME},
				KEY_ID + "=?",new String[]{String.valueOf(id) }, null,null,null,null);
	    	Log.d(LOG,c.toString());
	    	if(c.moveToFirst()){
	    		do{
	    			Log.d(LOG,"Adding dice" + get(c,KEY_NAME));
	    			w.addDice(Dice.valueOf(getString(c,KEY_NAME)));
	    		}while(c.moveToNext());
	    	}   		
	    	c.close();

            //Actions
            c = db.query(TABLE_WEAPON_ACTION, new String[]{KEY_ID,KEY_NAME,KEY_ATTRIBUTE1,KEY_ATTRIBUTE2,KEY_VALUE,KEY_FATIGUE},
                    KEY_ID + "=?",new String[]{String.valueOf(id) }, null,null,null,null);
            Log.d(LOG,c.toString());
            if(c.moveToFirst()){
                do{
                    Log.d(LOG,"Adding action " + getString(c, KEY_NAME)
                            ...
                            +get(c,KEY_VALUE)+","+get(c,KEY_FATIGUE));
                    w.setAction(Action.valueOf(getString(c, KEY_NAME)), Attribute.valueOf(getString(c,KEY_ATTRIBUTE1)),
                            Attribute.valueOf(getString(c,KEY_ATTRIBUTE2)),get(c,KEY_VALUE),get(c,KEY_FATIGUE));
                    //public void setAttribute(Action test,Attribute first, Attribute second,int value, int fatigue){
                }while(c.moveToNext());
            }
            c.close();
    ...
    public synchronized ArrayList<Armor> getAllArmor(){
        ...
        if(c.moveToFirst()){
            do{
                list.add(new Armor(getLong(c,KEY_ID),getString(c,KEY_NAME), BodyPart.valueOf(getString(c,KEY_BODYPART)),get(c, KEY_PROTECTION),get(c, KEY_WEIGHT)));
            }while(c.moveToNext());
        }
        db.close();
        c.close();
    ...
    public synchronized Armor getArmor(Long id) throws Exception{
        ...
        if(c.moveToFirst()){
            ...
            Armor armor = new Armor(getLong(c,KEY_ID),getString(c,KEY_NAME), BodyPart.valueOf(getString(c,KEY_BODYPART)),get(c,KEY_PROTECTION),get(c,KEY_WEIGHT));
	...
	private void multiPush(String table, Long id, String column,
			ArrayList<String> entrys, boolean update) {
		...
		if(update){
			for(String entry :entrys){
				...
				con.put(column, entry);
				db.update(table,con,KEY_ID+"=?", new String[]{String.valueOf(id)});
			}
		}else{
			for(String entry :entrys){
				con = new ContentValues();
