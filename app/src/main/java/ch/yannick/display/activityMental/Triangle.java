...
public class Triangle {
    ...
    public Triangle(Vector vec, float color) {
        float x = ((float)vec.getX())/40;
        float y = ((float)vec.getY())/40;
        triangleCoords = new float[]{   // in counterclockwise order:
            x,  y+0.12f, 0.0f, // top
                    x-0.1f, y, 0.0f, // bottom left
                    x+0.1f, y, 0.0f  // bottom right
        };

        this.color = new float[]{ color, color, color, 1.0f };
